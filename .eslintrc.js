...
module.exports = {
  root: true, // 停止对父文件夹的 eslint 配置文件的查找
  globals: {}, // 配置全局不用检测的变量
  parserOptions: {
    parser: 'babel-eslint', // 防止 和 babel-parser 和 vue-config-parser 解析器检测冲动
  ...
  plugins: ['vue'], // 使用 eslint-plugin-vue 检测 vue 的文件，注意这里可能会检测时候和 babel-eslint 检测冲突
  ...
  rules: {
    ...
    **/
    ...
    // 在定义对象或数组时，最后一项不能加逗号 always: 表示必须加, never: 表会不加
    ...
    // 规定callback 如果有 err 参数，只能写出 err 或者 error.
    ...
    'new-parens': 2,
    'no-var': 2, // 禁用 var，用 let 和 const 代替
